    ...
    <template slot="popover">
      <div ref="dropdownBody" class="dropdownBody border border-light rounded pb-2">
        <div class="searchField p-1 md:p-2">
          <zk-input v-model="dropdownSearch" class="bg-white" size="sm" spellcheck="false" :maxlength="10" placeholder="Search for token" @keyup.native="enter">
            <template slot="icon">
  ...
  computed: {
    ...
    displayedTokens(): ZkSingleToken[] {
      ...
      for (key in this.tokens) {
        if (Object.prototype.hasOwnProperty.call(this.tokens, key) && (!this.feeAllowed || (this.feeAllowed && this.tokens[key].enabledForFees))) {
